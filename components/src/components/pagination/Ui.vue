    ...
    <div class="pagination-controls">
      ...
      v-bind="$attrs">
        <template #superPrev="{disabled}">
          <slot name="superPrev" :disabled="disabled" :rtl="rtl"></slot>
            <button 
              ...
              :class="[roundedClass, sizeClass, textColorClass.active, bgColorClass.active]"
              :disabled="disabled"
            >
            <svg-icon type="mdi" :path="superPrevIcon"></svg-icon></button>
        </template>
        ...
        <template #default="{isCurrentPage, isActive, page}" >
          ...
          <span v-if="page === -1 && showDefaultPagination" class="dot mx-2" :class="textColorClass.onActive">...</span>
        </template>
        <template #next="{disabled}">
          ...
          <button 
            ...
            :disabled="disabled"
          >
          <svg-icon type="mdi" :path="nextIcon"></svg-icon></button>
        </template>
        <template #superNext="{disabled}">
          ...
          <button 
            ...
            :disabled="disabled"
          >
          <svg-icon type="mdi" :path="superNextIcon"></svg-icon></button>
        </template>
...
<script setup lang="ts">
  ...
  const superPrevIcon = computed(() => props.rtl ? mdiChevronDoubleRight : mdiChevronDoubleLeft);
  const prevIcon = computed(() => props.rtl ? mdiChevronRight : mdiChevronLeft);
  const superNextIcon = computed(() => props.rtl ? mdiChevronDoubleLeft : mdiChevronDoubleRight);
